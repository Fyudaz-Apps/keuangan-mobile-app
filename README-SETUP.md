...
A mobile personal finance app built with React Native and Expo. Track expenses, manage categories, set budgets, and maintain control over your personal finances with ease.
...
│   │   ├── geminiService.ts      # AI transaction parsing (Gemini / 9router fallback)
...
3. **Prebuild (required — Realm/expo-sqlite are native modules, Expo Go won't work)**
   ```bash
   npm run prebuild
   ```
...
- **`appStore`**: Global app state (theme, language, currency)
...
```env
# Gemini AI (only needed for the natural-language transaction input)
EXPO_PUBLIC_GEMINI_API_KEY=your_gemini_api_key_here

# 9router AI (local proxy fallback; used instead of Gemini when both are set)
...
The `docs/` directory is maintained to match the current SQLite-based code: `PRD-Keuangan-Mobile-App.md` (product requirements), `IMPLEMENTATION-CHECKLIST.md` (what's done vs not), `SETUP-SUMMARY.md`, `PROJECT-SETUP-COMPLETION.md`, and `GEMINI-INTEGRATION-GUIDE.md` (AI parsing). **This file remains the primary setup/architecture reference; trust code first.**
...
- **Gemini**: requires a valid `EXPO_PUBLIC_GEMINI_API_KEY` (`AIza...`); invalid keys surface as HTTP 400 `API_KEY_INVALID`.
